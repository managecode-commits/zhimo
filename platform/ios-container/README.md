...
应用图标已提供在 `Assets.xcassets/AppIcon.appiconset`。创建容器 App target 后，将该资源目录
加入 target，并设置 App Icon Source 为 `AppIcon`；不要将其只添加到键盘扩展 target。
...
`target/apple-xcframework/ZhimoCore.xcframework` 组合为 Xcode 工程。完整创建 target、
