...
class HandwritingPanelTest {
    @Test fun alphabetSwitchRetainsInkAndInvalidatesOldCandidates() {
        val instrumentation = InstrumentationRegistry.getInstrumentation()
        val context = instrumentation.targetContext
        val prefs = context.getSharedPreferences("zhimo",0)
        val image = prefs.getBoolean("handwriting_image_experimental",true)
        val hadImage = prefs.contains("handwriting_image_experimental")
        val oldMode = prefs.getString("handwriting_character_mode",null)
        val requests = mutableListOf<Pair<InkRequest,(Result<List<String>>) -> Unit>>()
        ...
        instrumentation.runOnMainSync {
            prefs.edit().putBoolean("handwriting_image_experimental",true).putString("handwriting_character_mode","MIXED").commit()
            candidates = LinearLayout(context)
            panel = HandwritingPanel(context,Color.BLACK,Color.WHITE,1f,candidates,commit={ commits.add(it);true },providerFactory={ language ->
                languages.add(language)
                object: HandwritingProvider {
                    override fun available(done:(Boolean)->Unit) = done(true)
                    override fun close() {}
                    override fun recognize(request:InkRequest,done:(Result<List<String>>)->Unit) { requests.add(request to done);arrived.countDown() }
                }
        ...
        try {
            ...
            instrumentation.runOnMainSync {
                val switch=(panel.getChildAt(0) as LinearLayout).getChildAt(0)
                val original=requests.first()
                switch.performClick() // Chinese while mixed result is pending
                original.second(Result.success(listOf("旧")))
                assertEquals(0,candidates.childCount)
                assertEquals(HandwritingCharacterMode.CHINESE,requests.last().first.characterMode)
                requests.last().second(Result.success(listOf("中", "天", "夫", "大", "夭", "未")))
                assertEquals(5, (0 until candidates.childCount).count {
                    candidates.getChildAt(it).contentDescription?.toString()?.startsWith("手写候选：") == true
                })
                val oldKey=candidates.getChildAt(0)
                switch.performClick() // digits
                assertEquals(HandwritingCharacterMode.DIGITS,requests.last().first.characterMode)
                assertEquals(original.first.strokes,requests.last().first.strokes)
                requests.last().second(Result.success(listOf("0")))
                assertEquals("0",(candidates.getChildAt(0) as android.widget.Button).text.toString())
                oldKey.performClick(); assertTrue(commits.isEmpty())
                switch.performClick() // letters
                assertEquals(HandwritingCharacterMode.LETTERS,requests.last().first.characterMode)
                assertEquals(original.first.strokes,requests.last().first.strokes)
                requests.last().second(Result.success(listOf("a","A")))
                candidates.getChildAt(0).performClick()
                assertEquals(listOf("a"),commits)
                assertFalse(panel.hasUncommitted())
                assertEquals(listOf("mixed-line","zh-CN-line","digits-line","letters-line"),languages)
            }
        } finally {
            instrumentation.runOnMainSync { panel.dispose() }
            prefs.edit().putString("handwriting_character_mode",oldMode).apply {
                if(hadImage) putBoolean("handwriting_image_experimental",image) else remove("handwriting_image_experimental")
            }.commit()
        }
    }
    @Test fun correctionReplacesOneCharacterWithoutEarlyCommitAndRejectsStaleButtons() {
        ...
        lateinit var panel: HandwritingPanel
        lateinit var candidates: LinearLayout
        lateinit var viewport: android.widget.HorizontalScrollView
        ...
        instrumentation.runOnMainSync {
            ...
            viewport = android.widget.HorizontalScrollView(instrumentation.targetContext).apply {
                addView(candidates, android.widget.FrameLayout.LayoutParams(-2, -1))
            }
            val width = (280 * instrumentation.targetContext.resources.displayMetrics.density).toInt()
            viewport.measure(View.MeasureSpec.makeMeasureSpec(width, View.MeasureSpec.EXACTLY), View.MeasureSpec.makeMeasureSpec(150, View.MeasureSpec.EXACTLY))
            viewport.layout(0, 0, width, 150)
        ...
        try {
            ...
            instrumentation.runOnMainSync {
                ...
                viewport.measure(View.MeasureSpec.makeMeasureSpec(viewport.width, View.MeasureSpec.EXACTLY), View.MeasureSpec.makeMeasureSpec(150, View.MeasureSpec.EXACTLY))
                viewport.layout(0, 0, viewport.measuredWidth, 150)
                val correction = find(candidates, "逐字纠正手写候选")!!
                assertTrue("Correction button must fit the narrow candidate viewport", correction.right <= viewport.width)
                assertEquals("从", (candidates.getChildAt(1) as android.widget.Button).text.toString())
                candidates.getChildAt(1).performLongClick()
                assertNotNull(find(panel, "纠正第1字：从"))
                assertNull(find(panel, "纠正第2字：左"))
                // Close correction without committing before testing the two-character hypothesis.
                find(panel, "返回手写候选")!!.performClick()
