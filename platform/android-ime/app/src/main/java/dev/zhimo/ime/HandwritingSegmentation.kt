...
object HandwritingSegmentation {
    ...
    fun plans(request: InkRequest): List<Plan> {
        ...
        // A roughly square ink envelope is compatible with a single Chinese
        // character even when radicals do not touch. Preserve all alternatives.
        val points = request.strokes.flatten()
        val inkHeight = (points.maxOf { it.y } - points.minOf { it.y }).coerceAtLeast(1f)
        return if (span / inkHeight <= 1.35f && request.characterMode != HandwritingCharacterMode.DIGITS &&
            request.characterMode != HandwritingCharacterMode.LETTERS)
            alternatives.sortedBy { if (it.groups.size == 1) 0 else 1 } else alternatives
    }
