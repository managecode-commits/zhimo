...
class KeyboardKeyView(context: Context) : Button(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (selectionIndicator && isSelected) {
            legendPaint.color = currentTextColor
            legendPaint.style = Paint.Style.FILL
            val d = resources.displayMetrics.density
            canvas.drawRoundRect(width * .3f, height - 5 * d, width * .7f, height - 2 * d, d, d, legendPaint)
        }
    }
