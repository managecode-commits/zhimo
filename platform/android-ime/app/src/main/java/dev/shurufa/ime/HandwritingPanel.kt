...
class HandwritingPanel(
    ...
    private val providerFactory: (String) -> HandwritingProvider = { language ->
        val single = if (context.getSharedPreferences("shurufa", 0).getBoolean("handwriting_image_experimental", true))
            ImageHandwritingProvider(context) else BundledHandwritingProvider(context)
        if (language == "zh-CN-line") LineHandwritingProvider(single) else single
    },
    private val onLiteral: (String) -> Unit = {},
    private val onDelete: () -> Unit = {},
    initialLineMode: Boolean = context.getSharedPreferences("shurufa", 0).getBoolean("handwriting_image_experimental", true),
    ...
    private fun loadProvider() {
        ...
        val selected = runCatching { providerFactory(if (lineMode) "zh-CN-line" else "zh-CN") }.getOrNull()
        provider = selected
        language.text = if (lineMode) "从左到右" else if (selected is ImageHandwritingProvider) "字形·实验" else "中文单字"
    ...
    private fun recognizeNow() {
        ...
        val request = session.snapshot(canvas.width.toFloat(), canvas.height.toFloat())
            ?.copy(boundaries = if (lineMode) manualBoundaries else null) ?: return
        val token = generation
        ...
        provider?.recognize(request) { result ->
            ...
            result.fold(onSuccess = { values ->
                if (session.accept(request.revision, values)) {
                    hideGrid()
    ...
    private fun showCandidates(revision: Long) {
        ...
        val cell = candidateWidth()
        val previewCount = if (lineMode) 2 else 5
        ...
        if (correctionChoices(revision).isNotEmpty()) candidateRow.addView(styledButton("逐字改", 14f).apply {
        ...
        status.text = when {
            ...
            expanded -> "上下滑动选字 · 不同字数备选排在后面"
            lineMode -> "优先完整词组 · 长按候选逐字改，字数不对可调整分字"
            else -> "点选上屏 · 没有合适的字可补笔"
        }
    }
    ...
    private fun correctionChoices(revision: Long): List<List<String>> {
        ...
        if (!session.active || session.revision != revision || line.characterRevision != revision ||
            line.characterChoices.any { it.isEmpty() }) return emptyList()
        return line.characterChoices
    }
    private fun startCorrection(revision: Long, value: String? = session.choices.firstOrNull()): Boolean {
        ...
        if (choices.isEmpty() || value == null) return false
    ...
    private fun showGrid(revision: Long) {
        ...
        expanded = true; canvas.visibility = View.INVISIBLE; grid.visibility = View.VISIBLE
        val rows = LinearLayout(context).apply { orientation = VERTICAL }
        val columns = if (lineMode) 1 else ((grid.width.takeIf { it > 0 } ?: (area.width - dp(52))) / candidateWidth()).coerceIn(3, 8)
    ...
    private fun readyHint() = if (lineMode) "从左到右连续写1至4字，不限格；可手动调整字界" else "写一个字，点选上屏"
