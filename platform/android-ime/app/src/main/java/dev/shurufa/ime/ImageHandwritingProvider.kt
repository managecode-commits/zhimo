...
class ImageHandwritingProvider(context: Context) : HandwritingProvider {
            ...
            val result = runCatching {
                ...
                OnnxTensor.createTensor(OrtEnvironment.getEnvironment(), FloatBuffer.wrap(input), longArrayOf(1, 3, 48, 320)).use { tensor ->
                    active.run(mapOf(active.inputNames.first() to tensor)).use { output ->
                        ...
                        decode(probabilities, characters)
                    }
                }
            }
    ...
    companion object {
        ...
        fun decode(frames: Array<FloatArray>, labels: List<String>): List<String> {
            ...
            // This mode is Chinese single-character input, not punctuation/OCR text lines.
            ...
            for (i in active.indices) {
                ...
                if (label.codePointCount(0, label.length) != 1 ||
                    Character.UnicodeScript.of(label.codePointAt(0)) != Character.UnicodeScript.HAN) continue
                val score = active[i] + ended[i]
                if (score > 0 && (best.size < 100 || score > (best.peek()?.second ?: Double.NEGATIVE_INFINITY))) {
                    best.add(label to score)
                    if (best.size > 100) best.poll()
                }
            }
            return best.sortedByDescending { it.second }.map { it.first }
        }
    }
}
