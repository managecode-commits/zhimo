...
最新连写移除固定三格，支持多个分字候选与手动字界，见 [不限格连写与手动分字](../../docs/不限格连写与手动分字.md)。

历史三格版本（已被不限格连写替代）：安装包与边界见 [三格横向连写](../../docs/三格横向连写.md)。

最新共用顶栏 APK：`app/build/outputs/apk/debug/app-debug-unified-topbar.apk`，有候选原位替换工具栏、无候选恢复；见 [共用顶栏记录](../../docs/多字停顿叠写.md#共用候选与工具顶栏)。

最新精简标题栏 APK 为 `app/build/outputs/apk/debug/app-debug-clean-header.apk`，说明见 [叠写与界面更新](../../docs/多字停顿叠写.md#后续界面精简包)。

新增 [多字停顿叠写](../../docs/多字停顿叠写.md)：手写面板点击“单字”切换，草稿可纠错，确认后整句上屏。

最新按键和手写参考布局、新 APK 及验收见 [参考键盘布局改版](../../docs/参考键盘布局改版.md)。

最新实验性离线字形模型、网格候选、启用开关和 APK 见 [手写字形模型与网格界面](../../docs/手写字形模型与网格界面.md)。

手写归一化、100 候选分页及最新修复包见 [手写识别质量整改](../../docs/手写识别质量整改.md)。

手写模型现已内置 APK，安装后无需下载。中文单字能力、模型许可、隐私边界和实测记录见 [内置离线手写交付](../../docs/内置离线手写交付.md)。
