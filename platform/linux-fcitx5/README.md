...
`$XDG_DATA_HOME/zhimo/user`（可用 `ZHIMO_USER_DATA_DIR` 覆盖）。实现依据 Fcitx 5
...
生产 librime 构建可通过 `ZHIMO_RIME_SHARED_DIR` 和 `ZHIMO_RIME_USER_DIR` 启用 `rime`
引擎；未提供时明确使用参考拼音。

```bash
cargo build -p ime-ffi
cmake -S platform/linux-fcitx5 -B target/fcitx5 \
  -DZHIMO_IME_LIBRARY="$PWD/target/debug/libime_ffi.so" \
