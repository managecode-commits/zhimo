...
推荐短描述：`Offline-first input method with Pinyin, handwriting and speech transcription. Rust core; Windows/Linux preview, Android development, macOS prototype.`

中文介绍：知墨 Zhimo 是一个开源、离线优先的输入法项目，将拼音、手写与语音转文字连接到可扩展的 Rust 底座。当前优先邀请 Windows、Ubuntu 和 Debian 用户参与预览测试。
...
- 介绍侧栏与 Topics 需 GitHub 连接恢复后再设置。当前文档不是“已完成侧栏配置”的声明。
...
> 我在开发开源输入法「知墨 Zhimo」：支持拼音、离线单字手写和录音后本地转文字，底层使用 Rust。现邀请 Windows、Ubuntu 24.04／Debian 12 用户测试桌面预览版。项目还在早期，手写准确率、Wayland 和应用兼容需要继续改进；欢迎带复现步骤的反馈。源码与版本：https://github.com/managecode-commits/zhimo
