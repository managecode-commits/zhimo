...
run_test() {
  local test_output
  test_output="$(adb shell am instrument -w "$@" dev.zhimo.ime.test/androidx.test.runner.AndroidJUnitRunner)"
  printf '%s\n' "$test_output"
...
run_test -e requireNativeRime true -e runKeyboardUi true \
  -e class dev.zhimo.ime.HandwritingSessionTest,dev.zhimo.ime.HandwritingPanelTest,dev.zhimo.ime.RuntimeSmokeTest,dev.zhimo.ime.KeyboardInteractionTest
run_test -e class dev.zhimo.ime.HandwritingModelTest
run_test -e runKeyboardUi true -e requireNativeRime true -e runRealInk true -e runLine true \
  -e class dev.zhimo.ime.KeyboardInteractionTest
run_test -e class dev.zhimo.ime.HandwritingImageModelTest,dev.zhimo.ime.LineHandwritingTest
