...
use super::{lexicon, normalized_t9_input, t9_signature, LexiconEntry};
...
fn forms(reading: &str, numeric: bool) -> Forms {
    reading
        ...
        .map(|s| {
            ...
            if numeric {
                values = values.into_iter().map(|v| t9_signature(&v)).collect();
            }
            values.sort();
            values.dedup();
            values
...
fn accepts(forms: &Forms, input: &str) -> bool {
    if forms.len() < 2 || input.len() >= 128 || forms.len() > 32 {
        return false;
    }
